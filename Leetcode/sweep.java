...
class Endpoint {
	public Endpoint(Double x, Double y, Integer lineNo, String type) {
		xCoordinate = x;
		yCoordinate = y;
		this.lineNo = lineNo;
		pointType = type;
...
public class sweep {
	...
	public static PriorityQueue<Endpoint> queueOfPoints;
	...
	public static void main(String[] args) {
		...
		queueOfPoints = new PriorityQueue<Endpoint>(100, new Comparator<Endpoint>() {
		...
		for(int i=0; i<numLines; i++) {
			...
			Endpoint p = new Endpoint(x1,y1,line,"left");
			queueOfPoints.add(p);
			Endpoint p1 = new Endpoint(x2,y2,line,"right");
	...
	public static void lineSweep(PriorityQueue<Endpoint>  queueOfPoints) {
		while(!queueOfPoints.isEmpty()) {
			Endpoint point = queueOfPoints.remove();	
	...
	///*** Check for intersection between two lines
		public static void checkIntersection(Double[] upper, Double[] lower) {
			...
			if(intersecting) {
				double slope = ((x4 - x3) * (y1 - y3) - (y4 - y3) * (x1 - x3))/((x1-x2)*(y3-y4) - (y1-y2)*(x3-x4));
				double intersectX = x1 + slope*(x2 - x1);
				double intersectY = (y1 + slope*(y2 - y1));
				System.out.printf("%.2f %.2f",intersectX,intersectY);
				System.out.println();
			}
		}
		...
		//** Check if the point is on line segment 
